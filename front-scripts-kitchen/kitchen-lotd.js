...
btn.addEventListener('click', async () => {
    ...
    try {
        ...
        const { data } = await axios.get(`https://app-beiu.herokuapp.com/api/v1/kitchenLogs/get/${month}-${day}-${year}`, {
                headers: {
        ...
        if(data.length == 0) {
            ...
            data.forEach( (item) => {
                ...
                sureBtn.addEventListener('click', async () => {
                    try {
                        const { data } = await axios.get(`https://app-beiu.herokuapp.com/api/v1/kitchenLogs/remove/${parag.getAttribute('data-id')}`, {
                            headers: {
                                Authorization: `Bearer ${token}`,
                            },   
                        })
